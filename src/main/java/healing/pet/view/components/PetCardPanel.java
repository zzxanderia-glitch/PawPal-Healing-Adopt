...
public class PetCardPanel extends JPanel {
    ...
    private void loadPetImage() {
        try {
            ...
            System.out.println("数据库中的 photoPath: " + path);
            ...
            if (is != null) {
                img = ImageIO.read(is);
                is.close();
                System.out.println("从 classpath 加载成功：images/" + path);
            }
            ...
            if (img == null) {
                ...
                if (imgFile.exists()) {
                    ...
                    System.out.println("图片文件不存在：" + imgFile.getAbsolutePath());
                }
            }
    ...
    private String getHealingLabel(Animal pet) {
        if (pet instanceof Cat) {
            ...
            if ("粘人".equals(personality)) return "软萌粘人精";
            if ("独立".equals(personality)) return "高冷小傲娇";
            return "治愈小猫咪";
        } else if (pet instanceof Dog) {
            String personality = ((Dog) pet).getPersonality();
            if ("活泼".equals(personality)) return "暖心小天使";
            if ("聪明".equals(personality)) return "机灵小伙伴";
            return "治愈小狗勾";
        }
        return "治愈小宝贝";
    }
